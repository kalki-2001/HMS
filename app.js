...
{
    res.render('signin_customer',{msg: false});
});

app.get('/alogin',function(req,res)
{
    res.render('signin_admin',{msg: false});
});
...
app.post('/loginuser', urlencodedParser, function (req, res) 
{
        mycon.query(`select * from Details where (email=? && password=?); `,[req.body.email,req.body.psw],function(err1,results)
        {
            ...
            if(results.length == 0)
            {
                res.render('signin_customer',{msg:"No user with this credentials"});
            }
            else
            { 
...
{
        mycon.query(`select * from Adminn where (email=? && password=?); `,[req.body.email,req.body.psw],function(err1,results)
        {
            console.log(results);
            if(results.length == 0)
            {
                res.render('signin_admin',{msg:"No admin with this credentials"});
            }
            else
            { 
                res.render('dashboard_admin',{result: results[0].username});             
            }
        });
}); 
